 ...
 *   B. BEHAVIOUR (win32 only) — spawn a console-less parent for real, and prove BOTH ways:
 *      a plain child gets a VISIBLE window (if it does not, this instrument is blind and says so
 *      instead of passing), and a child spawned with the shared option gets none.
 *
...
if (src !== null) {
  ...
  while ((m = CALLS.exec(src)) !== null) {
    ...
    if (!/windowsHide|NO_CONSOLE_WINDOW/.test(argsText)) bare.push(`${m[1]}() at sea_trial.mjs:${line}`);
  }
...
if (process.platform !== "win32") {
  ...
  // one child each way — the control that MUST show a window, and the fix that must not.
  fs.writeFileSync(path.join(dir, "parent.mjs"), `
import { spawn } from "node:child_process";
import fs from "node:fs";
const hold = ["-e", "setTimeout(()=>{},6000)"];
const bare = spawn("node", hold, {});
const hidden = spawn("node", hold, { windowsHide: true });
...
`;
  ...
  while (Date.now() - started < 12000) {
    try {
      ...
      if (pids.bare && pids.hidden) break;
    } catch { /* not written yet */ }
    execFileSync(process.execPath, ["-e", "setTimeout(()=>{},250)"], { windowsHide: true });
  }

  if (!pids.bare || !pids.hidden) {
    fail("the console-less parent never reported its children — the behaviour half did not run, so treat the coverage half above as the only evidence");
  } else {
    ...
    else pass("a child spawned with the option opens no window, and its own children inherit that");
  }

  for (const p of [pids.bare, pids.hidden, parent.pid]) { try { process.kill(Number(p)); } catch { /* already gone */ } }
  fs.closeSync(fd);
  fs.rmSync(dir, { recursive: true, force: true });
}
