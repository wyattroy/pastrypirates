...
async function leg(tag, w, h) {
  ...
  /* (A) DOES IT FOLLOW? One notch — the smallest gesture a player can make. A card that follows
     moves about as far as the notch; a card that slams goes the whole way. */
  await C.ev(WATCH);
  const NOTCH = 4;
  ...
  await sleep(1400);
  const followPts = JSON.parse(await C.ev(`JSON.stringify(window.__w34||[])`)).map(p => p[1]);
  const reached = followPts.length ? Math.max(...followPts) : 0;
  ...
  console.log(`     A. one ${NOTCH}px notch moved the card ${Math.round(reached)}px`);
  ...
  return { notch: NOTCH, reached, landEnd, overshoot, tracked, dragY: after.y, park: geom.parkDistance, easing: geom.easing };
}
...
for (const [tag, m] of out) {
  ...
  const slams = Math.abs(m.reached) > Math.max(m.notch * 4, m.park * 0.5);
  const bounces = m.overshoot > 2;
  const parked = Math.abs(m.landEnd - m.park) <= 2;
  if (slams || bounces || !parked || !m.tracked) bad++;
  console.log(`  ${tag}: one notch -> ${Math.round(m.reached)}px of ${m.park}px ${slams ? "SLAM" : "follows"}; on release it ${parked ? "parks" : "DOES NOT PARK (" + Math.round(m.landEnd) + "px)"}${bounces ? `, overshooting by ${Math.round(m.overshoot)}px` : " without a bounce"}; a finger still drags it ${m.tracked ? "live" : "NOT AT ALL"}`);
}
killAll();
