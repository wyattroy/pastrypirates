 ...
 *
 * 990 -> 350, 2026-09-08, by the daily doc audit: THE RATCHET WAS BORN ALREADY SLIPPED, and the
 * one step it depends on is the one step that did not happen. The trim (6733f743) took CLAUDE.md
 * 1359 -> 200 without touching this file; CEILING was then introduced two commits later
 ...
 * comment above called 990 a "high-water mark" it had never been, and the file was left free to
 * grow from 227 back to 990 with the build green the entire way — "growth back toward four
 * figures", the single thing .claude/DAILY-DOC-AUDIT.md exists to catch, with its detector
 * holding 763 lines of slack. Lowered to TARGET: still downward-only, still green today at 227,
 * and it leaves 123 lines of headroom so Wyatt's own next edit does not hit a wall. */
