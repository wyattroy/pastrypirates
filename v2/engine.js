...
let _ev = 0;
const E = (t, o) => Object.assign({ t, v: 1, id: ++_ev }, o);
...
export class GameV2 {
  ...
  ev(e) { this.events.push(Object.assign(e, { round: this.round })); return e; }
  ...
  *run() {
    ...
    this.ev(E("setup", { seats: this.players.map(p => ({ i: p.idx, name: p.name, power: p.power })) }));
    ...
    while (!this.over && this.round < 60) {
      ...
      this.windNext = DK[Math.floor(this.r() * 4)];
      this.stormNext = this.r() < STORM_CHANCE;
      this.ev(E("round", { n: this.round, wind: this.windNow, storm: this.stormNow, windNext: this.windNext, stormNext: this.stormNext }));
    ...
    this.ev(E("gameover", { order: this.finishOrder.slice(), standings: this.standings() }));
  }
  ...
  storm() {
    ...
    this.ev(E("storm", { dir: this.windNow, moves }));
  }

  *turn(p) {
    if (p.done) return;
    if (p.lostTurn) { p.lostTurn = false; this.ev(E("aground", { p: p.idx })); return; }
    this.ev(E("turn", { p: p.idx }));

    // ---- Phase 1: the trade outcry (free) ----
    yield* this.outcry(p);
    ...
    if (cells.length) {
      ...
      if (choice && (choice[0] !== p.pos[0] || choice[1] !== p.pos[1])) {
        ...
        this.ev(E("sail", { p: p.idx, from, to: [...p.pos], swept }));
      }
    }
  ...
  *resolveAct(p, act) {
    if (act.a === "bake") {
      p.done = true; this.finishOrder.push(p.idx); this.ev(E("bake", { p: p.idx })); return;
    }
    if (act.a === "dock") {
      const ing = act.ing, heads = this.flip();
      if (heads) p.coins += TREASURE;
      this.ev(E("treasure", { p: p.idx, ing, heads, got: heads ? TREASURE : 0 }));
      ...
      if (this.board.tokens[ing] > 0 && p.coins >= cost) {
        ...
        if (buy) {
          ...
          this.ev(E("buy", { p: p.idx, ing, cost }));
        }
      } else if (p.coins < cost) this.ev(E("broke", { p: p.idx, ing, cost }));
      return;
    }
    ...
    if (act.a === "poach") { p.coins += 2; this.ev(E("poach", { p: p.idx, got: 2 })); return; }
    this.ev(E("pass", { p: p.idx }));
  }
  ...
  *outcry(p) {
    ...
    this.ev(E("offer", { p: p.idx, want: offer.want, giveIng: offer.giveIng || null, giveCoins: offer.giveCoins || 0, sale: !!offer.sale }));
    ...
    if (!answers.length) {
      ...
      this.ev(E("nodeal", { p: p.idx, want: offer.want }));
      return;
    }
    const pick = yield { kind: "pick", seat: p.idx, options: { answers, offer } };
    if (pick === null || pick === undefined || pick < 0) { this.ev(E("walked", { p: p.idx })); return; }
    ...
    if (offer.sale) {                                    // I sell them my crate for their bid
      ...
      this.ev(E("trade", { a: p.idx, b: q.idx, gave: offer.want, got: pay + " coins", kind: "sale" }));
      ...
      this.ev(E("trade", { a: p.idx, b: q.idx, gave: offer.giveIng, got: offer.want, kind: "swap" }));
    } else {                                             // I buy with coin
      const pay = Math.min(deal.ask, p.coins);
      p.coins -= pay; q.coins += pay;
      q.ing.splice(q.ing.indexOf(offer.want), 1); p.ing.push(offer.want);
      this.ev(E("trade", { a: p.idx, b: q.idx, gave: pay + " coins", got: offer.want, kind: "buy" }));
    }
  ...
  *battle(att, def) {
    ...
    if (win.power === "shooter") win.coins += bets[win.idx];
    ...
    this.ev(E("battle", { a: att.idx, d: def.idx, ca: bets[att.idx], cd: bets[def.idx], downwind, how, flips, win: win.idx, spoil }));
  ...
  *cast(caller) {
    ...
    while (live.size) {
      ...
      for (const q of [...live]) {
        ...
        if (!stay) { q.coins += takes; took[q.idx] = takes; live.delete(q); }
      }
      if (!live.size) break;
      const heads = this.flip();
      ladder.push({ stage, pot, heads, still: [...live].map(q => q.idx) });
      if (heads) { stage++; if (stage > 10) { for (const q of live) { q.coins += Math.pow(2, stage); took[q.idx] = Math.pow(2, stage); } break; } }
      else {
        for (const q of live) {
          if (q.power === "trawler") { const t = Math.pow(2, stage + 1); q.coins += t; took[q.idx] = t; }   // decides after the coin lands
          else took[q.idx] = 0;
        }
        break;
      }
    }
    this.ev(E("cast", { caller: caller.idx, ladder, took }));
  }

  *finalLap(finisher) {
    this.ev(E("finallap", { p: finisher.idx }));
    for (const q of this.players) {
      if (q === finisher || q.done) continue;
      yield* this.turn(q);
      if (!this.needs(q).length && this.atHome(q) && !q.done) { q.done = true; this.finishOrder.push(q.idx); this.ev(E("bake", { p: q.idx })); }
    }
  }
