...
- the viewer's RECIPE sits in a band across the top of the captains panel — its name plus one ingredient per chip, with a green tick on the ones ye hold. Those chips are ingredients the recipe needs, never cargo. Wyatt ruled on 2026-09-12 that they wear no box and are never dimmed, because they sit on the recipe card itself; until that build lands they are still boxed and dimmed until held, and BOTH states are designed. Every ROW, the viewer's own included, shows only what that captain actually holds, or "empty hold";
...
- a captain's crates overlapping each other in the captains panel is DESIGNED (Wyatt, 2026-09-11): every hold stays on one line, and when the crates do not fit they slide together — up to 35% overlap, each with a soft edge — and past that the line scrolls sideways with a fade at its right edge. A fade or a half-covered crate there is never a clipping fault;
