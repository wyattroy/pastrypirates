...
| **A cloud CTO can PUBLISH to staging but cannot CHECK its own publish** — **DIAGNOSED 2026-08-27, and it was never the allowlist** | Wyatt allowlisted the host and it still failed, so it got measured properly. **`http://` is refused by the proxy no matter what is on the allowlist** (`Host not in allowlist`); **`https://` is ALLOWED** — the proxy answers `200 Connection Established` to the CONNECT — and then the request dies after the tunnel opens with **no HTTP status at all**. Production over `https://` returns 200 from the same container, so it is this host, not the network. **DNS is correct**: `getent hosts staging.playpastrypirates.com` names `wyattroy.github.io` and GitHub's Pages addresses. The repo's `CNAME` is correct too. | **The missing piece is the TLS CERTIFICATE, and it is the same root cause as Wyatt landing on production by accident.** The proxy re-terminates TLS, so the certificate `curl` validates is the PROXY's — a successful handshake says nothing about GitHub. An origin with no certificate is exactly what "tunnel opens, then silence" looks like from behind one. **FIX, in the staging repo's Settings → Pages:** clear the Custom domain, save, re-enter `staging.playpastrypirates.com`, save — that re-runs the DNS check and re-requests the certificate — then tick **Enforce HTTPS**. **A CAA record on the apex that omits `letsencrypt.org` blocks this permanently;** check that first if re-adding never issues one. Until then verify a publish through git, and note that `deploy-staging.sh` now polls for the stamp itself and prints both possibilities rather than one guess. |
