...
- **The trunk of this project is `claude/backlog-milestone-planning-93eb10`, not `main`** (Wyatt, 2026-07-31). `main` is a dead v1.0 snapshot — 489 commits behind, with no `src/` folder at all. **Never merge, rebase, or compare against `main`.** This worktree was brought current with the trunk on 2026-07-31 and now sits at trunk + this phase's commits. The phase-19 and phase-21 worktrees are still based on `origin/main` and so are genuinely stale relative to the trunk — bring them up to the trunk branch, never to `main`.
