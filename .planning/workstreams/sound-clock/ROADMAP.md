...
- [x] **Phase 21: Sound & the Clock Toggle** — SHIPPED 2026-08-02, live on playpastrypirates.com. Luis's sound effects on by default, a mute button, his credit — plus the timer on/off toggle working in solo and pass-and-play (AUDIO-01/02/03, FIX-02/N-03). Wyatt's full verification matrix passed, including the Safari storm-with-sound regression check.
  - [x] **DONE 2026-08-02 — the mute button's alignment, size, icon and touch labelling.** Placement is now MEASURED (`placeMuteButton()`, src/ui/board.js) rather than keyed on a proxy or a threshold: it asks whether the button plus the row's standard gap fits beside the turn clock, and moves it under the captains box when it does not. Also halved in size with a 44px tap area, given aria-label/aria-pressed (native `title` never appears on touch), and `sound-off.png` was rebuilt so the X is legible at 21px. See `.planning/todos/done/2026-08-01-mute-button-alignment-and-tooltips.md`. **This workstream has no open items.**
