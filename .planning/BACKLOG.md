...
| **P-3** | **A wind forecast chip is drawn on the board that no player can ever see.** `src/ui/board.js:402-416` builds a `FORECAST:` chip; `index.html:1905` hides it outright (`body.pp4Stage .fcChip{display:none}`), and `pp4Stage` is on the body for the whole of every game. So it is built, positioned and maintained on every render for nobody. | Measured 2026-08-27 while resolving W2-1's "pill or narration?" ambiguity. | Deleting UI is not a copy fix and it is not on the list. It is also possible this is deliberate — a surface kept for a layout that may return. **Wyatt's call: delete it, or say why it stays so the next reader stops re-finding it.** |
