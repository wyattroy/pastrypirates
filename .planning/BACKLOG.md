...
## Wave 3 — glitches a player sees constantly (6)
...
| **W4-8** | 🔴 **MEASURED 2026-09-12 — the fourth captain is cut off on a short phone.** At **390×664** the captain's box is clamped to **188px** while its four rows and the recipe band need **197px**, so the bottom row is clipped in the real game, not only in a mock-up. The clamp is `cap.style.setProperty("max-height", …)` in `src/ui/stage.js:3159` (the phone/tablet branch — *not* the `#pp4Col` rule at index.html:1765, which needs `pp4Side`). Measured by `scripts/qa/_capbox_shape.mjs`, which poses a real four-captain voyage at five screen sizes. **At 390×844 it fits with room to spare**, so this is short windows only. Found while measuring the box for the plaque art; not caused by it. |
