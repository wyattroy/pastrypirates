...
| **W3-7** | ✅ **BUILT 2026-09-12 — the tiny coin turns over another captain's boat, and the big coin is yours alone now.** Every number is his own from the coin tuner (`.claude/memory/DECISIONS.md`, 2026-09-10): 16px at 1x and it grows with the camera · 11px above the hull · no sideways nudge · rises off the boat in 40ms with the sound · turns 6 times a second with a 21px toss · lands on the blip at 795ms · 22% bounce · 6px glow · 13% shadow · holds 800ms · fades in 140ms. `src/ui/dockcoin.js` + `#dockCoinHost` (registered in `CAM_HTML_LAYERS`, so it rides the director's zoom). **THE DESIGN CALL INSIDE IT, and it is his to reverse:** a bot's dock no longer takes over the flippenator — the big coin is the one YOU tapped for, and four captains docking in a row used to seize the screen four times over something you had no say in. One line in `botDockCoin` puts it back. **His trap, honoured and measured:** the coin's promise does not resolve for 1595ms (795 + 800) and its one caller narrates on the line after the await, so nothing can announce the result over a spinning coin. Gate: `scripts/qa/w37_dock_coin_check.mjs`, ten checks, and the posed pictures are in `.planning/posed/dockcoin/`. |
| **W3-8** | 🔴 **ON A SLOW LOAD, THE RECIPE PICKER WAITS FOR ITS PICTURES.** His ruling, 2026-09-13: *"Yes, wait for the art."* Seen in Wy-Blade's full trial on 98c3dce5 (sep13-seatrial-blade-2, ea04d2c0): under ten concurrent loads the first picker screens drew recipe cards with only the name and divider — no dish picture, no ingredient icons — and the board behind had no island art either; on passplay-phone the autoplayer tapped "Bake this!" on a blank card. Later screens of the same voyages had the art, so it is a load race, not a broken card: boot()'s fresh-visit path (JOURNEY 1) deliberately does not await board art. **The fix is the gate, not a timer:** keep "Hoisting the sails" up (or hold the picker) until the recipe cards' pictures and the island art have DECODED — measured, never a fixed wait, and with a deadline that shows the picker anyway rather than stranding a player. Prove it with a throttled load in a probe: blank card before, full card after. |
