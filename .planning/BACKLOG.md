...
| ~~W3-5~~ | ✅ **CLOSED 2026-08-30 — FIXED, AND NOW WATCHED RATHER THAN READ.** A trade-wind square's preview stays on screen after you click a trade-wind square and then click a yellow sailing square. | **The fix landed 2026-08-29 (the teardown was nested inside `if (!cell)`, so it fired only when the tap missed EVERY sail square; it is unconditional now). A text gate has passed since — but its pass lines claimed BEHAVIOUR from SOURCE, the fault eleven CEO reviews have named, and nobody had watched it happen. MEASURED 2026-08-30 in a browser with real CDP mouse events (`scripts/qa/w35_sweep_preview_live.mjs`): tapping the trade-wind square drew all three parts — `sweepPath, sweepEnd, sweepGhost` — and tapping a plain yellow square left **0**. The text gate now names only text and points at the live one.** |
