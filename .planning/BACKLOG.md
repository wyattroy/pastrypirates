...
| # | Item | Gear | Note |
|---|---|---|---|
| ~~W0-1~~ | ✅ DONE (checked 2026-09-10) — `?endcard=1`, `?ovens=1` and `?bake2=1` each auto-start solo and land (src/orchestrator.js); they are the doors on his sheet. Original: **Skip straight to the endgame for testing.** A URL he can type on his phone that drops him into a bake-off SECOND attempt, and one for the End of Voyage card. | plumbing | **Four of his PROBLEM marks (#3, #4, #5, #6) are marked PROBLEM only because he could not get there.** Follows the existing `?bakeoff=1` / `?ovens=1` / `?wind=1` pattern (`src/shared/index.js:433,478`). ✅ **DECIDED 2026-08-27: staging and localhost ONLY** — never production. `devHost()` (`src/shared/index.js:465`) is localhost-only today; widen it to include `staging.playpastrypirates.com`. **A player on the live game must not be able to reach the end card by URL.** |
| ~~W0-2~~ | ✅ DONE (checked 2026-09-10) — the sheet's first tap copies; the dialog is confirmation ("THE FIRST TAP NOW COPIES" in its script). Original: **`Copy my notes` takes two clicks.** First opens the box; second copies. | n/a | `.planning/staging-checklist.html:160` builds the text and opens a `<dialog>`; `:172` does the copy. Make the first click do both, dialog becomes confirmation. |
| ~~W0-3~~ | ✅ DONE — the stamp reads `Build 2026.09.07.3`, staging appends `-staging@<sha>`, as decided 2026-08-27. Original: **Reformat the build stamp.** Currently `v4 · build 2026-08-26k-CUTOVER-STAGING/aug26-night-fixes@b8d61e42` — long, and "v4" no longer means anything. | cosmetic | **DECIDED 2026-08-27: date-based build number** — `Build 2026.08.27.3`, staging appends `-staging`. `src/ui/stage.js:33,1998`; the staging suffix is written by `scripts/deploy-staging.sh:171`. |
...
| **W3-6** | 🔴 **THE MUSIC RESTARTS FROM THE TOP EVERY TIME HE COMES BACK TO THE TAB.** His words, 2026-09-12: *"every time I leave the tab and come back, the song begins at the beginning again, and it's annoying and surprising and violates my expectations, which is that the song should pause when I pause the game, and it should pick back up where it left off."* **What he wants:** when the sound is paused because the game is backgrounded, remember where the music was in its ~7-minute cycle, and resume from that point when he comes back — not from zero. **Where it lives, so nobody rediscovers it:** the track deliberately does NOT loop (`src/ui/audio.js:177`) — it plays once, ends, a timer waits, and `musicPlayOnce()` starts it again from the beginning; `musicStop()` clears that timer and `musicStart()` calls `musicPlayOnce()` straight away. So **there is nowhere a position is kept** — restarting at zero is the design, not a bug in it, and the fix is to add the memory rather than to repair something broken. ⛔ **DO NOT REACH FOR WALL-CLOCK — his correction, and it is the opposite of the obvious instinct:** *"we don't need to advance the audio while context is suspended. That's actually the opposite of what I was saying. I want the audio to pause exactly where it was while the context is suspended and restart back exactly where it was when the context is unsuspended."* So the song must NOT catch up to where it would have been — it freezes and continues. `ctx.currentTime` stopping while suspended (`audio.js:482`) is therefore the RIGHT clock for this, not a trap: nothing accumulates while he is away, which is the behaviour he asked for. **The real trap is the node type:** the music is an `AudioBufferSourceNode` (`musicPlayOnce`, `audio.js:1157`), and a buffer source **cannot be paused** — once stopped it is dead. Resuming means keeping an `elapsed` figure across stops and calling `src.start(0, elapsed)` with the offset argument, and giving `MUSIC_GAP_SEC`'s waiting timer the same treatment so the gap resumes too rather than restarting. Read `docs/AUDIO.md` first; this is the file where a plausible-sounding change has bitten twice. |

| **W3-7** | 🔴 **THE TINY COIN FLIP FOR DOCKING BESIDE ANOTHER CAPTAIN'S BOAT — tuned by him, not built.** His settings, read back off the coin tuner and already recorded in `.claude/memory/DECISIONS.md` (2026-09-10): **16px at 1× zoom and grows with the camera · 11px above the boat · no sideways nudge · arrives by RISING off the boat in 40ms, starting with the sound · FLIPS OVER at 6 flips/s with a 21px toss, landing on the blip at 795ms · landing bounce 22% · glow 6px · holds its face 800ms · shadow 13% · leaves by FADING in 140ms.** Nothing of it is in `src/` yet. ⛔ **THE TRAP HE NAMED BEFORE ANYONE COULD FALL INTO IT, 2026-09-12:** *"the narration box comes up immediately after the sound of the coin flipping. And instead, you'll need to have that narration box showing the result of the coin flip wait until the coin flip is over."* The result line must not appear while the coin is still turning — the flip is the question and the narration is the answer, and an answer that lands first makes the animation decoration. **Budget it from his own numbers: the coin lands at 795ms and holds its face for 800ms, so the earliest honest moment for the narration is the landing, and the natural one is after the hold.** |
...
| **W4-9** | 🔴 **CEO-AUDITED 2026-09-12 — the BOARD is square; the white frame around it is not.** His report: *"the board is always supposed to be viewed in a square, and it looks like that square is slightly rectangular now, at least on mobile."* Measured at nine viewports across three builds: **the board itself is square everywhere** — the SVG's screen matrix has identical x and y scale to five decimals at every size, and a grid square renders **47.247 × 47.247px**. What is not square is the framed window: **375 × 421 on his iPhone, 46px taller than a square**, and 375 × 517 at full screen. **Cause:** `CAP_BASE = Math.min(250, S.capNeed, squareRoom)` (`src/ui/stage.js:894`) is a **floor, not an equality** — the 2026-08-23 fix stops the strip being SHORTER than a square and says nothing about taller. Derived and then verified at four heights: the window is square only while the visible page height is at most **board width + 295px**; above that it is exactly `height − width − 295` taller. And the white rounded outline he photographed is `#board`'s own border (`index.html:153` + `:1851`), which traces the **element box — the whole strip — not the square board inside it**. **NOT a regression:** identical at `dd56bb96` and `be8a5208`, and 4px closer to square on today's `dev`. **Two ways to fix, his call:** (a) move the frame so it traces the square board and let the extra sea sit outside it, or (b) cap the strip at a square and letterbox, which costs sea on a tall phone. Full verdict in `.planning/CEO-REVIEWS.md`; evidence in `scratchpad/shots/board-square-audit.json`. |
...
- [x] ✅ settled by his 2026-09-10 ruling above (the finding was the radial fan's known churn,
      re-reported as new every trial). Original: ~~31% of screens never settle~~ before being checked — all now reporting `churn: geometry`
...
- [x] ✅ **HE RULED, 2026-09-10** — *"you can drop this entirely from the sea trial — i'm honestly
      confused by what you're even seeing. the boat is never moving when the radial fans appear."*
      The unsettled-screen finding is gone from the verdict (scripts/lib/leg_verdict.mjs says so,
      with his words); waitSettled() still WAITS, it just no longer reports. The original entry:
