...
- [x] **PAR-10**: *(Group B)* The first screen is free of faults visible at a glance — the greyed Attack prompt draws as the same radial fan as every other action, a battle frames the two boats fighting rather than a bystander's, the ovens-be-roarin' block is centred both ways, no popup appears until the camera and the ships have stopped moving, and a laptop shows the phone's picture so Wyatt and the session debug the same screen *(items 2, 3, 5, 11, 22 — 22 is a stopgap; the desktop redesign stays in Phase 8)*
- [x] **PAR-11**: *(Group C)* A bot that reaches Tortuga with a full recipe fires the ovens and forfeits the pass dubloon, exactly as a human in that position must — in the browser game and in the headless simulator alike, and proven from the event stream rather than asserted *(item 4)*
- [x] **PAR-12**: *(Group C′)* Wyatt has read a table of purses over time, battle counts and voyage length across today's economy and each of the settings he named, from a few hundred games each — **and no economy number in the shipping game has changed, because he asked for the numbers, not for the change** *(item 12)*
...
| PAR-10 | Phase 02.2 — Wyatt's Twenty-Two (inserted) | Complete — Group B, plan 02.2-03. Items 2, 3, 5 measured not to reproduce; item 11 fixed; item 22 shipped as a reversible stopgap |
| PAR-11 | Phase 02.2 — Wyatt's Twenty-Two (inserted) | Complete — Group C, plan 02.2-04. canBake(p) guards both bot fallback doPass sites; bot_bake_pass_check.js red-proofed (99 violations pre-fix, 0 post-fix) |
| PAR-12 | Phase 02.2 — Wyatt's Twenty-Two (inserted) | Complete — Group C′, plan 02.2-05. scripts/economy_table.js red-proofed against bot_ladder4.js; 02.2-ECONOMY-TABLE.md delivered (300 games/cell); no game code changed, no PP4_STAMP bumped; Wyatt asked to pick |
