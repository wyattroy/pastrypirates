...
- [x] **Phase 18: Prompts & Polish** — ✅ SHIPPED 2026-08-02, live. — Buttons wait for the typewriter; a narrow window stops clipping the only button that takes the action; narration stops jumping sideways as it fades and the box stops shrinking under a still-fading line; orange buttons restyled; captain circles removed; no orphaned coins or brackets (FIX-03, FIX-04, FIX-06, FIX-07, FIX-08, FIX-09, FIX-10, FIX-16, FIX-17, FIX-21)
- [x] **Phase 19: Safari Check** — ✅ PASSED, merged. — **GATE.** Prove an always-on wind layer is safe in real Safari, and learn the dot-count budget, before Phase 20 invests. Builds a throwaway-able prototype; ships nothing final (WIND-00)
- [ ] **⬜ Phase 20: The Board Comes Alive — THE ONLY REMAINING v1.3 WORK.** — Drifting wind dots, arrows flowing into a rotating whirlpool, a signal before a ship is swept into the trade winds, and a pastry scent on every wind direction change (WIND-01…05). **Depends on Phase 19 passing**
- [x] **Phase 21: Sound & the Clock Toggle** — ✅ SHIPPED 2026-08-02, live, no open items (the mute button was the last one). — Luis's sound effects on by default, a mute button, his credit — plus the timer on/off toggle finally working in solo and pass-and-play (AUDIO-01/02/03, FIX-02/N-03)
- [x] **Phase 22: The Front Door** — ✅ SHIPPED 2026-08-02, live, no open items. — You name yourself after choosing how to play, a real About page, and a Google preview image (FIX-01, ABOUT-01/02, META-01). **Plus LOAD-03 — the front door's speed**, shipped 2026-08-02: a static blurred backdrop in place of a live game rendered only to be hidden, and a boot that decides the journey *before* painting — a first-time visitor gets the home screen in ~170ms, a mid-game refresh never sees the welcome screen at all. **COMPLETE 2026-08-02** — the last piece landed too: `renderDecorativeBoard()` was replaced by `seedIdleGameState()`, so the welcome screen now builds no board, no ships and no captain rows while still holding up the codebase-wide `appState.game` invariant (269 reads, 52 guarded). Idle cost 11.1% CPU / 60 layouts-per-sec -> 1.7% / 0 (`todos/done/2026-08-02-welcome-should-not-construct-a-game.md`)
