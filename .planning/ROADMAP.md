...
- [ ] **Phase 22: The Front Door** — You name yourself after choosing how to play, a real About page, and a Google preview image (FIX-01, ABOUT-01/02, META-01). **Plus LOAD-03 — the front door's speed**, shipped 2026-08-02: a static blurred backdrop in place of a live game rendered only to be hidden, and a boot that decides the journey *before* painting — a first-time visitor gets the home screen in ~170ms, a mid-game refresh never sees the welcome screen at all. **One piece still open:** `boot()` still *constructs* a whole decorative game it never shows — `todos/pending/2026-08-02-welcome-should-not-construct-a-game.md`, blocked on unguarded `appState.game` reads in `panel.js`
