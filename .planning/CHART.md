...
- [ ] **Sail the three Safari legs, which have never once run on this machine.** Fixed 2026-09-01 04:00Z: the WebKit browser was never installed here (documented step, never run) AND `openWebKit()` handed a raw Windows path to `import()`, which ESM reads as the protocol `c:` and rejects — so it reported "playwright not found" while playwright was installed and importable. Verified by launching: **WebKit now opens on the Razer.** ⚠ THE TRIAL RUNNING RIGHT NOW CANNOT BENEFIT — it started 03:07:33Z and loaded the old module, so its WebKit legs will still report NOT RUN through nothing but timing. When it finishes, sail them alone rather than re-running ninety minutes of Chromium:
  ⚠ **DELETE THE THREE CACHED FAILURE RECORDS FIRST, or they will be replayed instead of re-sailed.**
  `readDone()` resumes any leg holding a record at the current build stamp and does NOT check
  whether that record was a success — and these three now hold the very "playwright not found"
  failure the fix removes. Same resume-cache trap that wasted a run at 01:16Z tonight, in a
  different hat. Verified: `sea-trial-shots/legs/solo-desktop-wk--2026.08.31.2.json` records
  `leg error: playwright not found. Tried: C:\Users\wyatt\.pw\...` — exactly the raw-path bug.
  ```
  rm sea-trial-shots/legs/solo-*-wk--2026.08.31.2.json
  node scripts/playtest_gate.mjs --legs=solo-desktop-wk,solo-phone-wk,solo-tablet-wk --out=sea-trial-shots --judge=off
  ```
  Safari is a stated core requirement of this game, so the merge evidence is incomplete without it. *(Do not start it while the current trial is still sailing: both would write `.planning/wyclau/LONG-RUN`, and the second would overwrite the first's progress — the marker assumes one long job at a time, which is worth fixing if two ever need to run together.)*
