...
- **Wyatt, LIVE BUG REPORT, 2026-08-31 21:00Z, two screenshots**: *"after I send something to you in the ideas box, the page css breaks; and i'm not sure if the idea was sent. i need to be able to send another idea immediately afterwards, without waiting. i need to know that my first idea was sent, and added to the chart."* → **SHIPPED 2026-08-31 21:04Z.** Root cause measured, not guessed: a comment inside the client `<script>` block's own text read `// The state block is a JSON <script>, so...` — a literal, unescaped, tag-shaped substring sitting inside the real script element's content. Once the Ideas box's self-publish path (`buildDoc()`) re-embedded the whole page as a string and republished it, that substring survived and corrupted the render — confirmed by simulating the exact client-side escaping in Node and rendering the result in a real browser before and after. Fixed the comment; also fixed the Send button, which never re-enabled and never confirmed on success (relied entirely on the platform's own view reload, which his report shows isn't reliable enough): it now updates local state, clears the box, shows "Saved to the page — a session will harvest it to the Chart soon.", and re-enables immediately — matching, not overclaiming, what the page can actually promise (a session still has to harvest to the real Chart). Same fix applied to the rulings-save flow for consistency (rule 8). New gate `scripts/qa/glass_script_tag_purity_check.mjs`, checks the INSIDE of both real script blocks for stray tag-shaped substrings — red-proofed twice: a first version of the gate itself stripped the defect away before checking and falsely passed against the real bug, caught and fixed before trusting it; the corrected version then genuinely failed against the planted original bug and passed once fixed. npm test 73/73.
