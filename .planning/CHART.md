...
|---|---|---|
| **Staging cannot be deployed from the Razer: `rsync` does not exist on Windows.** The trial is done and green (10/10 legs finished) and the next step in your own ruling — *"deploy to staging for you to play"* — is blocked one command in: `scripts/deploy-staging.sh` line 133 is `rsync -a --delete "${EXCLUDES[@]}"`, and there is no rsync in git-bash. `tar` and `robocopy` are both present, so a substitute is possible. | **I did NOT substitute one, deliberately, and I want you to pick.** That script's own header records TWO sessions coming within one command of taking `playpastrypirates.com` down by hand-rolling this exact copy, and rule 14 says deploy with the script only, never hand-roll the sync — swapping its transport at 5am with no reviewer is that move wearing a different hat. The 45-entry exclude list is what keeps `CNAME` out of the staging repo, and rsync's pattern semantics are not tar's or robocopy's. **Three ways, my recommendation first:** (a) **install rsync on the Razer** (git-for-windows can add it, or MSYS2) — nothing safety-critical changes, and every future deploy from this machine just works; (b) deploy from a machine that already has rsync; (c) I add a guarded fallback inside the script that reuses the SAME exclude array, with the existing CNAME guard still doing the final check — doable, but it is surgery on the one file whose failure takes the live game down, and it should have a reviewer. | 2026-09-01 06:05Z |
| **The untappable sail square: four days, seven probes, zero fixes — is this still worth more session time right now?** A guest on a phone, in a crew game, sometimes gets one sail option drawn ~23-24px off the edge of the screen (real, reproduced repeatedly, magnitude consistent, direction not). Two full geometry theories are measured dead, `camFitCells`'s own math is proven sound with no browser needed, a fourth lead (a stale camera transform vs `#boardwrap`'s own width) is written up but not measured, and a same-seed probe just showed the bug's SHAPE varies turn to turn (so "reproduce it" alone no longer proves a fix). Counted, not felt (a fresh session's own audit): seven `scripts/qa/*.mjs` probes committed 2026-08-29 through today, zero commits to `stage.js`/`flow.js` carrying an actual placement/framing fix in that window. | **Recommendation: timebox it.** One more session gets the one measurement that would actually settle the strongest live lead (`#boardwrap.getBoundingClientRect().width` vs `vwPx()` at the moment of a real failing guest prompt — a single live read, not another theory) — if it comes back equal, that lead is dead too and this should stop being probed ad hoc and become a real GSD phase (or wait for Wyatt to reproduce it live and hand over a screenshot per rule 26, which has settled harder questions than this one in minutes). Not my call whether "one sail square, sometimes, on a phone, in crew mode" is worth more open-ended session time against the launch line right now — that weighing is yours. | 2026-09-01 11:35Z |
