...
overrides_applied: 0
human_verification:
  - test: "Re-run a clean two-browser-tab full in-game multiplayer smoke test (host creates room, guest joins with a distinct pp_id, host starts the voyage, host makes a move, guest observes it live via the narr/ev/prompt/flip/battle watchers, at least one battle or trade round completes) with no test-driving misclicks."
    expected: "Guest tab renders the host's moves in real time through the extracted src/net/ watchers; host authority is unchanged (guest never runs the engine); no console errors from the registry (no unexpected duplicate-attach refusals, no missing teardown)."
    why_human: "Requires two live browser tabs against a real Firebase RTDB room and a full turn-by-turn playthrough — cannot be scripted headlessly. The prior attempt (09-05 Transcript B) proved bidirectional lobby/seat sync cleanly but did not cleanly complete the in-game turn-propagation leg because of a coordinator UI misclick, not a code defect. Already tracked as an open item in WINDOWS.md and scheduled to close at Phase 12's VERIFY-03, but ROADMAP Phase 9 criterion 4 itself remains only partially demonstrated until this is observed cleanly."
---
...
| 4 | A multiplayer game across two browser tabs still syncs deterministically after the extraction (host + guest smoke test passes) | ⚠️ PARTIAL — see Human Verification | `09-05-SUMMARY.md` Transcript B (coordinator, two live Chrome tabs, live Firebase room ZXZS) proves distinct-identity join (D-11's shared-`localStorage` `pp_id` trap correctly avoided) and bidirectional lobby/seat-list sync through the extracted `seats` watcher — both tabs correctly reflected each other's presence. It does **not** cleanly prove the full in-game turn-propagation leg (host move → guest sees it via `narr`/`ev`/`prompt`/`flip`/`battle` watchers) — the coordinator's own defensive UI misclick dropped the host tab into pass-and-play mode mid-observation, an honestly documented test-driving artifact rather than a code finding. 09-05-PLAN.md's own must-have truth #6 ("the guest sees the host's events, narration, flips, battles and chat... ROADMAP criterion 4") was therefore not fully satisfied by this phase's own stated bar. |

**Score:** 3/4 truths verified, 1 partially demonstrated (routed to human verification, not counted as failed — see below).
