...
- **D-15:** The module entry sets a marker (`window.__pp_module_ok = true`) so the browser-side contract is machine-checkable from a Chrome MCP session, not just eyeballed. **Amended after research:** the assignment MUST be guarded with `typeof window !== "undefined"`. Unguarded it throws `ReferenceError: window is not defined` when the same module is imported under plain Node — which would break Success Criterion 1 ("imports identically in Node and the browser"), the very thing this phase exists to prove. Same guard applies to the D-17 `firebase` assertion.
