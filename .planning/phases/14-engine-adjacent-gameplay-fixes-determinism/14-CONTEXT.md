...
- **D-12:** **The false "the dock held fast" message must fire at the correct square.** Per STORM-01 — it currently appears while the boat is still a square away from the dock. The `moored` event fires from the island/home checks in the push loop (`src/engine/index.js:265-267`, mirrored at `src/ui/flow.js:212-216`). — **⚠ SUPERSEDED: see D-21 and D-22.** Research + live repro established the stated cause is wrong. The message is *truthful*; it is narrated **before** the board redraws, so the boat visibly hasn't arrived yet (D-22). The separate wording imprecision — one line covering three different causes — is fixed by D-21. The success criterion still stands; the mechanism does not.
...
### Decisions added during planning (2026-07-26, post-research)

These were settled with Wyatt after `14-RESEARCH.md` landed. Several **supersede or correct** earlier decisions — where they conflict, these win.

- **D-18: Tortuga becomes real land for wind purposes.** `leeward()` (`src/engine/index.js:255-257`) only checks `isIsland()`, and home is not in `this.islands` — so Tortuga casts **no wind shadow** today, where every other island does. Wyatt's intent: *"tortuga itself behaves like all other land squares in the game (it has leeward wind effects, it cannot be moved over or landed upon)."* Blocking-movement parity already exists (`:295`, `:338`, `:497` OR `isIsland(o)||isHome(o)`); the **leeward gap is real and must be closed**. Engine-tier change. — **Reversibility: one-way** — this is the single largest fixture-perturbing change in the phase (see D-22).
- **D-19: Tortuga's berths are docks, and they steady you.** The `man(p.pos,this.home)<=1` clause in `moored()` (`src/engine/index.js:253`) is **not** an overreaching patch — Manhattan distance ≤1 from home is *exactly* the four berths (the same squares the win check uses at `:737`). It is the code expressing "you're on a Tortuga berth, so the storm can't blow you into land," which is correct under D-18's "Tortuga docks behave the same as all other docks." **Keep this clause.** A ship parked at home can never be wrecked by a storm.
  - **Geometric consequence (settled explicitly):** Tortuga is a single square, so the only cells you can be pushed onto it *from* are its four orthogonal neighbours — which are exactly the berths. Therefore "run aground on Tortuga" and "berth protection" are mutually exclusive; whichever is kept, the other **never** fires. Wyatt chose berth protection. **There is no reachable aground-on-Tortuga case.** The `isHome(nx)` safe-harbor special case (`src/engine/index.js:265`, mirrored `src/ui/flow.js:212`) is therefore redundant with the berth clause and should be folded into normal handling rather than kept as a separate rule.
- **D-20: The passive dock save stays — mechanics unchanged.** A ship blown *onto* a dock square by the first square of a gust IS sheltered by that dock on the next square of the same gust (`moored()` → `adjPort(p)!==null` evaluates against the ship's already-updated position). Wyatt reviewed this deliberately and chose to keep it: *"let's keep the mechanics as-is, to give you a lucky save from being blown inland; but we'll change the narration text to explain it."* **No rules change here — narration only.**
- **D-21: The `moored` narration splits by cause (supersedes D-12's framing).** One line (`EVENT_NARRATION.moored`, `src/ui/util.js:253` — *"The dock steadies {p} from running aground ⚓"*) currently covers three distinct situations. Tag the `moored` event with a `reason` at each call site (`src/engine/index.js:265`, `:267`; `src/ui/flow.js:212`, `:216`) and render three honest lines: (a) docked deliberately last turn, (b) **steadied against a dock the storm put you on** — Wyatt's own wording: *"you're able to steady your boat against the dock to not be blown aground"*, (c) moored at Tortuga. **Adding a field to the event changes the serialized JSON and therefore every fixture SHA** — must land in the same re-record pass as D-18/D-15.
- **D-22: The reported "false dock held fast" symptom is a render-ordering bug, and D-12's stated cause was wrong.** `windLeg` (`src/ui/flow.js:216`) fires the event → `await narrateLastEvent()` → **then** `liveRender()`. Moving onto a plain water square does no intermediate render (`:262`). So the message plays while the board still shows the boat at its *previous* square — exactly the reported "message fires while the boat is still a square away from the dock." **The message was truthful; the picture had not caught up.** Per-square rendering (D-09) fixes the reported symptom on its own; D-21 fixes the separate wording imprecision.
- **D-23: D-17 is answered — no simulator misalignment, one re-record not two.** `takeTurn` (`src/engine/index.js:693-735`) calls `chooseAction()` exactly once per turn with hard `return`s on each branch; a simulated bot cannot trade *and* separately act. The engine already obeys the one-action rule. **Nothing to mirror; do not add a "trade+action" fix to the re-record.**
- **D-24: An offer on the table spends the action (resolves the hail-attempt-vs-success question).** Verified human precedent: `humanTrade` returns `false` **only** when the human backs out *before* an offer is made (`src/ui/flow.js:336`); every path where an offer was actually presented — accepted, declined, or countered-and-walked-away — returns `true`, and `humanAct` (`:485`) only re-opens the action menu on `false`. So the human rule is **"putting an offer on the table costs your action,"** not "a completed trade costs your action." **The bot's hail follows the same rule: it costs the action whether the human accepts or refuses.** Add a closing narration line for the refused-hail case so the bot visibly *paying* that price is legible (D-01) rather than reading as a glitch.
- **D-25: The hail stays UI-tier (resolves the Claude's-Discretion question about `chooseAction`).** `chooseAction` is a **shared** engine method called by both `botTurn` (`src/ui/flow.js:613`) and the simulator's `takeTurn` (`src/engine/index.js:725`). Folding the hail into it would inject hailing into the deterministic engine — directly violating D-08. **Correct shape:** keep the hail where it lives (`src/ui/flow.js:585-612`) and make a taken hail `return` before reaching `chooseAction()` at `:613`.
- **D-26: D-16's "confirm the differences are only storm-related" criterion is no longer achievable — replace it, do not drop it.** D-18's leeward change alters sail budgets near home, which changes bot routing, which cascades into `dock`/`trade`/`fish`/`battle` events. Non-storm divergence is now *legitimate*, so the original narrow check would fail for correct reasons. **Replacement criterion:** prove divergence is *explainable* rather than *narrow* — at minimum, (a) a full per-seed diff enumerating **every** divergent event across all 30 seeds (the existing `verify()` reports only the *first* divergence, `scripts/determinism_baseline.js:150-241` — a real tooling gap that blocks safe re-recording), and (b) an assertion that no seed diverges **before its first storm round**. D-16's spirit — *re-record deliberately, never reflexively; document what changed and why* — is unchanged and still binding.
- **D-27: D-13's premise was too strong — most storm copy already exists.** `EVENT_NARRATION` (`src/ui/util.js:225-323`) already has lines for every event in D-11's list (`windmove`, `blownOut`, `dodge`, `anchor`, `moored`, `blocked`, `anchorHold`, `aground`, `shipwrecked`), all written in shared third-person that reads correctly for a bot subject. The gap is **surfacing** them per-square for bots, not authoring them. **Reframes D-14's approval gate:** present Wyatt the existing lines reused as-is and ask whether to punch them up, *plus* the genuinely new copy — the three `moored` variants (D-21) and the refused-hail turn-end line (D-24).
